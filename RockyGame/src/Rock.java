...
public class Rock extends Polygon {
    ...
    public static int[] sPolyXArray = {9,17,26,34,27,36,26,14,8,1,5,1,10};
    public static int[] sPolyYArray = {0,5,1,8,15,20,31,28,31,22,16,7,0};

    public boolean onScreen = true;
    public boolean canBetouch = false;

    String explodeFile = "file:./src/boom.wav";
    ...
    public void move(SpaceShip theShip, ArrayList<PhotonTorpedo> torpedos) {
        ...
        for(Rock rock : rocks) {
            if(rock.onScreen) {
                ...
                if(theShip.getXCenter() > (width / 2) + 50 || theShip.getXCenter() < (width / 2) - 50 &&
                        theShip.getYCenter() > (height / 2) + 50 || theShip.getYCenter() < (height / 2) - 50) {
                    canBetouch = true; }
                else { canBetouch = false; }

                if(otherRock.intersects(shipBox) && canBetouch) {
